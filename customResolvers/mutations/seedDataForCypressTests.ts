    ...
    return async (parent: any, args: Args, context: any, resolveInfo: any) => {
      ...
      try {
        for (const serverConfig of serverConfigs) {
          if (serverConfig.serverName && serverConfig.DefaultServerRole?.connect) {
            const defaultServerRoleName =
              serverConfig?.DefaultServerRole?.connect?.where?.node?.name || "Cypress Test Server";
            ...
            await ServerConfig.update({
              ...
              update: {
                DefaultServerRole: {
                  connect: {
                    where: {
                      node: { name: defaultServerRoleName },
                    },
                    overwrite: true,
                  },
                },
