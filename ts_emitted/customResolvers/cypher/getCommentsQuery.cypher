...
WITH c, author, serverRole, parent, child, upvoter, $modName AS modName

OPTIONAL MATCH (c)<-[:HAS_FEEDBACK_COMMENT]-(feedbackComment:Comment)<-[:AUTHORED_COMMENT]-(feedbackAuthor:ModerationProfile)

WITH c, author, serverRole, parent, child, upvoter, modName, feedbackComment, feedbackAuthor,
...
WITH c, author, serverRole, parent,
...
WITH c, author, serverRole, parent, UpvotedByUsers, parentIds, weightedVotesCount, ageInMonths,
    [comment IN NonFilteredChildComments WHERE comment.id IS NOT NULL] AS ChildComments,
    FeedbackComments

WITH c, author, serverRole, parent, UpvotedByUsers, parentIds, ChildComments, FeedbackComments, ageInMonths, weightedVotesCount,
...
WITH c, author, serverRole, parent, UpvotedByUsers, parentIds, ChildComments, FeedbackComments, ageInMonths, weightedVotesCount, hotRank,
